...
public partial class LoginMain : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static string btnIppLogin(string userName, string password, string flag)
    {
        try
        {
            msg = "";
            msg = loginMethod("IPP", userName, password, flag);
            return msg;
        }
        catch (Exception ex)
        {
    ...
    public static string loginMethod(string type, string userName, string password, string flag)
    {
        ...
        {
            msg = "Connection failed. Please contact the administrator.";
        }
        return msg;
    }
    ...
    public static string[] GetUserDetails()
    {
        if (HttpContext.Current.Session["UserId"] == null)
        {
            ...
            return new string[] { "SESSION_EXPIRED" };
        }
        string[] result = new string[2];
        string Username = HttpContext.Current.Session["USERNAME"].ToString() ?? "";
        string ProfileImagePath =  HttpContext.Current.Session["PROFILE_FULL_PATH"].ToString() ?? "";
        string PROFILE_IMAGE_NAME = HttpContext.Current.Session["PROFILE_IMAGE_NAME"].ToString() ?? "";

        // Ensure path is root-relative
        if (!string.IsNullOrEmpty(ProfileImagePath) && !ProfileImagePath.StartsWith("/"))
        {
            ProfileImagePath = "/" + ProfileImagePath;
        }

        string fileFullPath = HttpContext.Current.Server.MapPath("~/assets/Images/ProfileImages/") + PROFILE_IMAGE_NAME;

        if (!File.Exists(fileFullPath) || string.IsNullOrWhiteSpace(PROFILE_IMAGE_NAME))
        {
            PROFILE_IMAGE_NAME = "DefaultUser.jpg";
        }

        string imageUrl = "/assets/Images/ProfileImages/" + PROFILE_IMAGE_NAME;
